...
the member name. */
select distinct Facilities.name, concat(Members.firstname, Members.surname) as member_name from Members Inner join Bookings on Bookings.memid=Members.memid inner join Facilities on Facilities.facid = Bookings.facid where Facilities.name like 'Tennis Court%' order by member_name;
...
Select Facilities.name as name, Concat(Members.firstname, Members.surname)as member_name, Bookings.starttime as time
...
SELECT *
FROM ( SELECT Facilities.name AS name, CONCAT( Members.firstname, Members.surname ) AS member_name, 
...
INNER JOIN Members ON Bookings.memid = Members.memid
WHERE Bookings.starttime LIKE '2012-09-14%') AS innerTable 
WHERE cost >30;
